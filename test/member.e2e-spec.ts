...
describe('R03 - Member', () => {
    ...
    describe('R03_C1 - Create member', () => {
        ...
        it('R03_C1_02 - Create member with invalid phone number', async () => {
            let create_member_invalid_phone = create_student_dto
    ...
    describe('R03_C2 - Find member', () => {
        it('R03_C2_01 - Find created member by ID', async () => {
            const { body } = await request(app.getHttpServer()).get('/member/' + member_id).auth(employee_token, { type: 'bearer' }).expect(200);
    ...
    describe('R03_C3 - Update member', () => {
        it('R03_C3_01 - Update created member: phone number', () => {
            return request(app.getHttpServer()).patch('/member/' + member_id).auth(employee_token, { type: 'bearer' }).send({ loan_permission: update_member.body.loan_permission }).expect(200);
        })

        it('R03_C3_02 - Update created member: loan permission', () => {
            return request(app.getHttpServer()).patch('/member/' + member_id).auth(employee_token, { type: 'bearer' }).send({ phone_number: update_member.body.phone_number }).expect(200);
        })
    })
    ...
    describe('R03_C4 - Delete member', () => {
        ...
        it('R03_C4_01 - Delete created member by ID', () => {
            return request(app.getHttpServer()).delete('/member/' + member_id).auth(employee_token, { type: 'bearer' }).expect(200);
        })
    })
});
