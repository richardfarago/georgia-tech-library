...
import { create_loan_bundle_library, create_loan_bundle_limit_reached, create_loan_bundle_professor, create_loan_bundle_student, create_loan_bundle_unavailable_book, create_loan_bundle_unloanable_book } from '../src/common/utilities/test-data/loan.test-data';
...
describe('R06 - Loan', () => {
    ...
    beforeAll(async () => {
        ...
        const student_response = await request(app.getHttpServer()).get('/member/' + student_auth.id).auth(chief_auth.token, { type: 'bearer' })
        student = student_response.body
        //Prof PRE -> 0 active loans
        const professor_response = await request(app.getHttpServer()).get('/member/' + professor_auth.id).auth(chief_auth.token, { type: 'bearer' })
        professor = professor_response.body
        //Lib PRE -> 0 active loans
        const library_response = await request(app.getHttpServer()).get('/member/' + library_auth.id).auth(chief_auth.token, { type: 'bearer' })
    ...
    describe('R06_C1 - Create loan', () => {
        ...
        it('R06_C1_01 - Create loan (Student)', async () => {
            const { body } = await request(app.getHttpServer()).post('/loan').auth(student_auth.token, { type: 'bearer' }).send(create_loan_bundle_student).expect(201)
            student_loan_id = body.id
        })

        it('R06_C1_02 - Create loan (Professor)', async () => {
            const { body } = await request(app.getHttpServer()).post('/loan').auth(professor_auth.token, { type: 'bearer' }).send(create_loan_bundle_professor).expect(201)
            professor_loan_id = body.id
        })

        it('R06_C1_03 - Create loan (Library)', async () => {
            const { body } = await request(app.getHttpServer()).post('/loan').auth(library_auth.token, { type: 'bearer' }).send(create_loan_bundle_library).expect(201)
            library_loan_id = body.id
        })

        it('R06_C1_04 - Create loan exceed book limit', () => {
            return request(app.getHttpServer()).post('/loan').auth(student_auth.token, { type: 'bearer' }).send(create_loan_bundle_limit_reached).expect(500).expect('{"statusCode":500,"message":"Book limit reached.","error":"Internal Server Error"}')
        })

        it('R06_C1_05 - Create loan with unavailable book instance', () => {
            return request(app.getHttpServer()).post('/loan').auth(student_auth.token, { type: 'bearer' }).send(create_loan_bundle_unavailable_book).expect(500).expect('{"statusCode":500,"message":"Book(s) currently not available.","error":"Internal Server Error"}')
        })

        it('R06_C1_06 - Create loan with unloanable book title', () => {
            return request(app.getHttpServer()).post('/loan').auth(student_auth.token, { type: 'bearer' }).send(create_loan_bundle_unloanable_book).expect(500).expect('{"statusCode":500,"message":"Book(s) are not loanable.","error":"Internal Server Error"}')
        })
    })
    ...
    describe('R06_C3 - Validate loan permissions (end date/due date)', () => {
        ...
        it('R06_C3_01 - Student (21 days loan period & 7 days grace period)', async () => {
            const { body } = await request(app.getHttpServer()).get(`/loan/${student_loan_id}`).auth(student_auth.token, { type: 'bearer' }).expect(200)

            let loan_period = moment(body.end_date).diff(moment(body.start_date), 'days')
            let grace_period = moment(body.due_date).diff(moment(body.end_date), 'days')
        ...
        it('R06_C3_02 - Professor (90 days loan period & 14 days grace period)', async () => {
            const { body } = await request(app.getHttpServer()).get(`/loan/${professor_loan_id}`).auth(professor_auth.token, { type: 'bearer' }).expect(200)
        ...
        it('R06_C3_03 - Library (100 days loan period & 70 days grace period)', async () => {
            const { body } = await request(app.getHttpServer()).get(`/loan/${library_loan_id}`).auth(library_auth.token, { type: 'bearer' }).expect(200)
    ...
    describe('R06_C4 - Return books', () => {
        ...
        it('R06_C4_01 - Return books (Student)', async () => {
            ...
            for (let i = 0; i < create_loan_bundle_student.loan_contents.length; i++) {
                promises.push(request(app.getHttpServer()).put(`/loan/${student_loan_id}/${create_loan_bundle_student.loan_contents[i].book_id}`).auth(student_auth.token, { type: 'bearer' }).expect(200))
            }
        ...
        it('R06_C4_02 - Return books (Professor)', async () => {
            ...
            for (let i = 0; i < create_loan_bundle_professor.loan_contents.length; i++) {
                promises.push(request(app.getHttpServer()).put(`/loan/${professor_loan_id}/${create_loan_bundle_professor.loan_contents[i].book_id}`).auth(professor_auth.token, { type: 'bearer' }).expect(200))
            }
